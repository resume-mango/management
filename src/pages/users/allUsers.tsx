...
import { apiBulkExportUsersLink } from "../../apis/users";
...
const AllUsers = () => {
  ...
  const bulkExportUsersLink = async () => {
    setExporting(true);
    const { data, error } = await apiBulkExportUsersLink();
    if (error) {
      setNotify({ type: "danger", heading: "Failed to export users" });
    }
  ...
  return (
    <Fragment>
      <DashPageHeader title="Users" border>
        <Button size="lg" onClick={bulkExportUsersLink} disabled={exporting}>
          {exporting ? (
            <div>
              <LoadingDots color="white">Exporting Users</LoadingDots>
            </div>
          ) : (
            "Export Users"
          )}
